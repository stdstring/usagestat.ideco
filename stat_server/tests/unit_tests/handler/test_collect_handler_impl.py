...
class TestCollectHandlerImpl(TestCase):
    ...
    def test_normal_life_cycle(self):
        self._logger.info('collect({0:s}) enter'.format(self._good_xml_packet))
        self._storage.save_data(self._good_data_packet)
        self._logger.info('collect({0:s}) exit'.format(self._good_xml_packet))
    ...
    def test_exception_in_deserialization(self):
        bad_xml_packet = '<data_packet id="83cf01c6-2284-11e2-9494-08002703af71">' +\
                         ...
                         '<data_item><source>source2</source><category>cat2</category><timemarker>2013-01-01 11:11:11</timemarker><data>IDKFA</data></data_item>' +\
                         '</data_packet>'
        self._logger.info('collect({0:s}) enter'.format(bad_xml_packet))
        self._logger.exception('exception in collect({0:s})'.format(bad_xml_packet))
    ...
    def test_exception_when_save(self):
        ...
        self._logger.exception('exception in collect({0:s})'.format(self._good_xml_packet))
